...
public class ConexionService {
    ...
    public List<Saga> buscarSagas() {
        try {
            ResponseEntity<List<Saga>> resp = rest.exchange(
    ...
    public boolean borrarSaga(Long id) {
        try {
            ...
            rest.exchange(url, HttpMethod.DELETE, null, Void.class);
            return true;
        } catch (HttpClientErrorException e) {
    ...
    public Saga crearSaga(Saga s) {
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            HttpEntity<Saga> req = new HttpEntity<>(s, headers);
            ResponseEntity<Saga> resp = rest.exchange(
    ...
    public Saga sagaPorID(Long id) {
        try {
            String url = PG_SAGA_URL + "/" + id;
            ResponseEntity<Saga> resp = rest.exchange(url, HttpMethod.GET, null, Saga.class);
            return resp.getBody();
        } catch (HttpClientErrorException e) {
